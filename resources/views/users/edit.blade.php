        ...
        <div class="px-4 mx-auto max-w-7xl sm:px-6 md:px-8">
            <form id="workoutForm" method="post" action="{{ route('users.update', ['user' => $user->id]) }}" onsubmit="sendWorkouts()" class="space-y-8 divide-y divide-gray-200">
                ...
                <div class="mt-6 space-y-6 sm:mt-5 sm:space-y-5">
                    ...
                    @if($user->role === "Client")
                        <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                            <label for="plan" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2">
                                Plano
                            </label>
                            <div class="mt-1 sm:mt-0 sm:col-span-2">
                                <select id="plan" name="plan" class="@error('plan') border-red-500 @enderror max-w-lg block focus:ring-green-500 focus:border-green-500 w-full shadow-sm sm:max-w-xs sm:text-sm border-gray-300 rounded-md">
